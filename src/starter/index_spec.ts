...
describe('starter', () => {
  ...
  it('works', async () => {
    expect(appTree.files).toContain(
      '/projects/' + nameProject + '/src/app/app.module.ts'
    );
  });
  ...
  it('commit lint: adds commitlint.config.js file', async () => {
    expect(appTree.files).toContain(
      '/projects/' + nameProject + '/commitlint.config.js'
    );
  });
  ...
  it('adds staging configurations to angular.json', () => {
    ...
    expect(
      workspaceConfig.projects[nameProject].architect.build.configurations
        .staging
    ).toBeDefined();
...
describe('starter white label', () => {
  ...
  it('should contain: tenant, configuration, theme, api responses and tsconfigs files', async () => {
    return schematicRunner
      .runSchematicAsync('ng-add', { 'white-label': true }, appTree)
      .toPromise()
      .then(() => {
        expect(appTree.files).toContain(
          '/projects/' +
            nameProject +
            '/src/testing/fakes/api-responses/get-config.json'
        );
        expect(appTree.files).toContain(
          '/projects/' +
            nameProject +
        ...
        expect(appTree.files).toContain(
          '/projects/' +
            nameProject +
            '/src/app/core/services/configuration/configuration.ts'
        );
        expect(appTree.files).toContain(
          '/projects/' +
            nameProject +
